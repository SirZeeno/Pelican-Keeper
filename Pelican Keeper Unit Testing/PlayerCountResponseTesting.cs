    ...
    public async Task PlayerCountResponseTest()
    {
        ...
        if (_ip != null && _port != null && _messageFormat != null)
            switch (_messageFormat)
            {
                ...
                {
                    var a2S = new A2SService(_ip, (int)_port);
                ...
                case CommandExecutionMethod.Rcon:
                {
                    ...
                    {
                        var rcon = new RconService(_ip, (int)_port, _password);
                        await rcon.Connect();
                        response = await rcon.SendCommandAsync(_command, null);
                    }
                    else
                    {
                        ConsoleExt.WriteLine($"Password or Command is null. Password: {_password}, Command: {_command}",
                            ConsoleExt.CurrentStep.None, ConsoleExt.OutputType.Info, null, true);
                    }
                ...
                {
                    var javaMinecraftQuery = new JavaMinecraftQueryService(_ip, (int)_port);
                ...
                {
                    var bedrockMinecraftQuery = new BedrockMinecraftQueryService(_ip, (int)_port);
        ...
        if (!string.IsNullOrEmpty(response))
        {
            ...
            {
                ConsoleExt.WriteLine("Success! The Response Conforms to the output Standard!",
                    ConsoleExt.CurrentStep.None, ConsoleExt.OutputType.Info, null, true);
                ConsoleExt.WriteLine($"Response: {response}", ConsoleExt.CurrentStep.None, ConsoleExt.OutputType.Info,
                    null, true);
                ConsoleExt.WriteLine($"Clean Response: {cleanResponse}", ConsoleExt.CurrentStep.None,
                    ConsoleExt.OutputType.Info, null, true);
                Assert.Pass($"{response}, {cleanResponse}\n");
            }
            else
            {
                ConsoleExt.WriteLine("Failed! The Response does not Conform to the output Standard!",
                    ConsoleExt.CurrentStep.None, ConsoleExt.OutputType.Info, null, true);
                ConsoleExt.WriteLine($"Response: {response}", ConsoleExt.CurrentStep.None, ConsoleExt.OutputType.Info,
                    null, true);
                ConsoleExt.WriteLine($"Clean Response: {cleanResponse}", ConsoleExt.CurrentStep.None,
