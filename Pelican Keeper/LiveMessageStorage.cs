...
public static class LiveMessageStorage
{
    ...
    private static async Task<bool> MessageExistsAsync(List<DiscordChannel> channels, ulong messageId)
    {
        ...
        foreach (var channel in channels)
            ...
            {
                WriteLine($"Message {messageId} not found in #{channel.Name}", CurrentStep.MessageHistory,
                    OutputType.Warning);
            }
    ...
    {
        var lastMessageId = Cache.LiveStore?.LastOrDefault(x => MessageExistsAsync([channel], x).Result);
    ...
    {
        var lastPaginated = Cache.PaginatedLiveStore?.LastOrDefault(x => MessageExistsAsync([channel], x.Key).Result);
