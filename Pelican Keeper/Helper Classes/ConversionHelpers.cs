...
public static class ConversionHelpers
{
    ...
    {
        return string.IsNullOrEmpty(value) ? "null" : value is "0" or "0.00" or "0 B" ? "∞" : value;
    }
    ...
    {
        var maxPlayerCount = "Unknown";

        if (string.IsNullOrEmpty(serverResponse) && maxPlayers > 0) return $"N/A/{maxPlayers}";

        if (string.IsNullOrEmpty(serverResponse)) return "N/A";

        if (maxPlayers != 0) maxPlayerCount = maxPlayers.ToString();
