...
public static class ExtractorHelpers
{
    ...
    public static int ExtractPlayerCount(string? serverResponse, string? regexPattern = null)
    {
        ...
        {
            ConsoleExt.WriteLine("The Response of the Server was Empty or Null!", ConsoleExt.CurrentStep.Helper,
        ...
        if (!serverResponse.Any(char.IsDigit))
            return
                0; // No digits found in the server response, so player count is 0 (probably a "no players" message or connection error/timeout)
        ...
        {
            ConsoleExt.WriteLine($"Player count extracted using standard format: {playerCount}",
                ConsoleExt.CurrentStep.Helper);
            return playerCount;
        }
        ...
        {
            ConsoleExt.WriteLine($"Player count extracted using Ark format: {arkRconPlayerList.Count}",
                ConsoleExt.CurrentStep.Helper);
        ...
        {
            ConsoleExt.WriteLine($"Player count extracted using Palworld format: {palworldPlayerList.Length}",
                ConsoleExt.CurrentStep.Helper);
        ...
        {
            ConsoleExt.WriteLine($"Player count extracted using Factorio format: {factorioPlayerCount}",
                ConsoleExt.CurrentStep.Helper);
        ...
        if (regexPattern != null)
        {
            ...
            {
                if (!int.TryParse(customMatch.Value, out var count)) return count;
        ...
        ConsoleExt.WriteLine("The Bot was unable to determine the Player Count of the Server!",
            ConsoleExt.CurrentStep.Helper, ConsoleExt.OutputType.Error, new Exception(serverResponse));
        return 0;
    }
    ...
    private static ServerAllocation?
        GetConnectableAllocation(
            ServerInfo serverInfo) //TODO: I need more logic here to determine the best allocation to use and to determine the right port if the main port is not the joining port, for example in ark se its the query port
    {
        if (serverInfo.Allocations == null || serverInfo.Allocations.Count == 0)
            ConsoleExt.WriteLine("Empty allocations for server: " + serverInfo.Name, ConsoleExt.CurrentStep.Helper,
                ConsoleExt.OutputType.Warning);
        return serverInfo.Allocations?.FirstOrDefault(allocation => allocation.IsDefault) ??
               serverInfo.Allocations?.FirstOrDefault();
    }

    /// <summary>
    ///     Determines if the IP is Internal or External and returns the Internal one if it's Internal and the Secrets
    ///     specified External one if it doesn't match the Internal structure.
    /// </summary>
    /// <param name="serverInfo">ServerInfo of the Server</param>
    ...
    public static string GetCorrectIp(ServerInfo serverInfo)
    {
        ...
        if (allocation == null)
        {
            ConsoleExt.WriteLine("[GetCorrectIp] No connectable allocation found for server: " + serverInfo.Name,
                ConsoleExt.CurrentStep.Helper, ConsoleExt.OutputType.Error);
        ...
        if (Program.Config.InternalIpStructure != null)
        {
            ...
            if (Regex.Match(allocation.Ip, internalIpPattern) is { Success: true }) return allocation.Ip;
        }
            ...
            ServerInfo serverInfo) //TODO: mat-pandaz was having issues here when it came to his servers not displaying the connectable IP
    {
        ...
        if (allocation == null)
        {
            ConsoleExt.WriteLine(
                "[GetReadableConnectableAddress] No connectable allocation found for server: " + serverInfo.Name,
                ConsoleExt.CurrentStep.Helper, ConsoleExt.OutputType.Error);
            return "No Connectable Address";
        }
