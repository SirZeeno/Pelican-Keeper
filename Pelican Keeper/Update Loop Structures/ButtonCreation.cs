...
public static class ButtonCreation
{
    ...
    {
        var selectedServerUuids = uuids;

        if (Config.AllowServerStartup is { Length: > 0 } &&
            !string.Equals(Config.AllowServerStartup[0], "UUIDS HERE", StringComparison.Ordinal))
        {
        ...
        if (Config.AllowServerStopping is { Length: > 0 } &&
            !string.Equals(Config.AllowServerStopping[0], "UUIDS HERE", StringComparison.Ordinal))
        ...
        if (Config is { AllowUserServerStartup: true, IgnoreOfflineServers: false })
        {
            var startOptions = selectedServerUuids.Select((uuid, i) =>
                new DiscordSelectComponentOption(
                    Program.GlobalServerInfo[i].Name, // shown to user
                    uuid // data you read on interaction
                )
            );
        ...
        if (Config.AllowUserServerStopping)
        {
            var stopOptions = selectedServerUuids.Select((uuid, i) =>
                new DiscordSelectComponentOption(
                    Program.GlobalServerInfo[i].Name,
                    uuid
                )
            );
    ...
    public static List<DiscordComponent> PaginatedButtonCreation(List<string?> uuids, int index)
    {
        if (index < 0 || index >= uuids.Count)
            WriteLine("Page Index out of range: " + index, CurrentStep.DiscordInteraction, OutputType.Error,
                new ArgumentOutOfRangeException(nameof(index)), true, true);

        // treat "UUIDS HERE" placeholder or empty/null list as "allow all"
        var allowAllStart = Config.AllowServerStartup == null || Config.AllowServerStartup.Length == 0 ||
                            string.Equals(Config.AllowServerStartup[0], "UUIDS HERE", StringComparison.Ordinal);
        ...
        var showStart =
            Config is { AllowUserServerStartup: true, IgnoreOfflineServers: false, AllowServerStartup: not null } &&
            (allowAllStart || Config.AllowServerStartup.Contains(uuids[index], StringComparer.OrdinalIgnoreCase));
        WriteLine("show Start: " + showStart, CurrentStep.DiscordInteraction, OutputType.Debug);

        // treat "UUIDS HERE" placeholder or empty/null list as "allow all"
        var allowAllStop = Config.AllowServerStopping == null || Config.AllowServerStopping.Length == 0 ||
                           string.Equals(Config.AllowServerStopping[0], "UUIDS HERE", StringComparison.Ordinal);
        ...
        var showStop =
            Config is { AllowUserServerStopping: true, IgnoreOfflineServers: false, AllowServerStopping: not null } &&
            (allowAllStop || Config.AllowServerStopping.Contains(uuids[index], StringComparer.OrdinalIgnoreCase));
        ...
        if (showStop)
            buttons.Add(new DiscordButtonComponent(ButtonStyle.Primary, $"Stop: {uuids[index]}",
                "Stop")); // The CustomId can never be the same.
    ...
    public static List<DiscordComponent> PerServerButtonCreation(string? uuid)
    {
        ...
        var showStart =
            Config is { AllowUserServerStartup: true, IgnoreOfflineServers: false, AllowServerStartup: not null } &&
            (allowAllStart || Config.AllowServerStartup.Contains(uuid, StringComparer.OrdinalIgnoreCase));
        WriteLine("show Start: " + showStart, CurrentStep.DiscordInteraction, OutputType.Debug);
        ...
        var showStop =
            Config is { AllowUserServerStopping: true, IgnoreOfflineServers: false, AllowServerStopping: not null } &&
            (allowAllStop || Config.AllowServerStopping.Contains(uuid, StringComparer.OrdinalIgnoreCase));
        WriteLine("show Stop: " + showStop, CurrentStep.DiscordInteraction, OutputType.Debug);
