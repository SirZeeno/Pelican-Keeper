...
public static class Paginated
{
    ...
    private static async Task SendPaginatedMessageAsync(this DiscordChannel channel, List<DiscordEmbed> embeds,
        List<string?> uuids)
    {
        var config = Program.Config;
        var lastMessage = LiveMessageStorage.TryGetLastPaginated(channel);
        var index = 0;
        if (lastMessage != null) index = lastMessage.Value.Value;
        var allEmbedsPassed = true;
        ...
        foreach (var embed in embeds)
        {
            ...
            WriteLine($"The Embed for {embed.Title} Failed Its Size Check", CurrentStep.DiscordMessage,
                OutputType.Error);
        }

        var buttons = ButtonCreation.PaginatedButtonCreation(uuids, index);
        ...
        if (lastMessage != null && !config.DryRun && allEmbedsPassed)
        {
            ...
            {
                WriteLine($"Updating paginated message {lastMessage.Value.Key} on page {currentIndex}",
        ...
        {
            WriteLine("Not every Embed passed its size check. Message Not Sent!", CurrentStep.DiscordMessage,
                OutputType.Error);
        }

        if (message != null) LiveMessageStorage.Save(message.Id, index);
    }
}