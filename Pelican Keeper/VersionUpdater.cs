...
public static class VersionUpdater
{
    ...
    public static async Task UpdateProgram()
    {
        ...
        var targetPath =
            await DownloadLatestAssetIfNewerAsync(
                CurrentVersion); // Downloads the appropriate version of the latest update for your platform if there is one available.
        if (string.IsNullOrEmpty(targetPath))
        {
            ConsoleExt.WriteLine("No update to apply.", ConsoleExt.CurrentStep.Updater);
            if (Directory.Exists(Path.Combine(Environment.CurrentDirectory, "old"))) await ReadOldConfigs();
            return;
        }
        ...
        var scriptPath = Path.Combine(Environment.CurrentDirectory, "Update Scripts", "Bare Metal", "update.sh");
        ...
        var appName = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
            ? "Pelican-Keeper.exe"
            : "Pelican-Keeper"; // no .exe on Linux or macOS, so I need to dynamically change the RID

        var psi = new ProcessStartInfo
        {
            FileName = "bash",
            Arguments =
                $"\"{scriptPath}\" \"{targetPath}\" \"{Environment.CurrentDirectory}\" {currentPid} \"{appName}\"",
    ...
    {
        var oldDirectory = Path.Combine(Environment.CurrentDirectory, "old");

        if (!Directory.Exists(oldDirectory)) Directory.CreateDirectory(oldDirectory);
        ...
        foreach (var file in files)
        {
            ...
            ConsoleExt.WriteLine(
                $"Moving {file.Name} to {Path.Combine(oldDirectory, file.Name)}. If the File already exists in the Directory, it will be overwritten.",
                ConsoleExt.CurrentStep.Updater);
        }
    }
    ...
    private static async Task<string?> DownloadLatestAssetIfNewerAsync(string currentVersionString)
    {
        ...
        if (!hasUpdate)
        {
            ConsoleExt.WriteLine($"No update. Current={currentVersionString}, latest={latestTag}",
                ConsoleExt.CurrentStep.Updater);
            return null;
        }

        ConsoleExt.WriteLine($"Update available! Current={currentVersionString}, latest={latestTag}",
            ConsoleExt.CurrentStep.Updater);

        if (release?.Assets == null || release.Assets.Length == 0)
        {
            ConsoleExt.WriteLine("No assets on latest release.", ConsoleExt.CurrentStep.Updater,
        ...
        var asset = Array.Find(release.Assets,
            a => a.Name.EndsWith($"{rid}.zip",
                StringComparison.OrdinalIgnoreCase)); // File format: Pelican-Keeper-vX.Y.Z-{rid}.zip

        if (asset == null)
        {
            ConsoleExt.WriteLine($"No asset found for RID '{rid}'.", ConsoleExt.CurrentStep.Updater,
                ConsoleExt.OutputType.Error);
            return null;
        }

        ConsoleExt.WriteLine($"Downloading {asset.Name} from {asset.BrowserDownloadUrl}",
            ConsoleExt.CurrentStep.Updater);
    ...
    private static async Task<(bool hasUpdate, string latestTag, Version? latestVersion, GitHubRelease? release)>
        CheckForUpdateAsync(string currentVersionString)
    {
        ...
        {
            var different = !string.Equals(latestTag, currentVersionString, StringComparison.OrdinalIgnoreCase);
    ...
    {
        var oldSecrets = await FileManager.ReadSecretsFile(Path.Combine(Environment.CurrentDirectory, "old"));
        var oldConfig = await FileManager.ReadConfigFile(Path.Combine(Environment.CurrentDirectory, "old"));

        await FileManager.ReadSecretsFile(); // Creating the Default Secrets file

        await File.WriteAllTextAsync(Path.Combine(Environment.CurrentDirectory, "Secrets.json"),
            JsonSerializer.Serialize(oldSecrets, new JsonSerializerOptions
            {
                WriteIndented = true
            }));

        await File.WriteAllTextAsync(Path.Combine(Environment.CurrentDirectory, "Config.json"),
            JsonSerializer.Serialize(oldConfig, new JsonSerializerOptions
            {
                WriteIndented = true
            }));
    }
