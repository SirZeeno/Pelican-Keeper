...
public static class DiscordInteractions
{
    ...
    internal static Task OnMessageDeleted(DiscordClient sender, MessageDeleteEventArgs e)
    {
        ...
        {
            WriteLine(
                $"Live message {e.Message.Id} deleted in channel {e.Message.Channel.Name}. Removing from storage.",
                CurrentStep.MessageHistory, OutputType.Debug);
        ...
        else if (liveMessageTracked == null)
        {
            ...
            WriteLine(
                $"Paginated message {e.Message.Id} deleted in channel {e.Message.Channel.Name}. Removing from storage.",
                CurrentStep.MessageHistory, OutputType.Debug);
    ...
    internal static async Task<Task> OnPageFlipInteraction(DiscordClient sender, ComponentInteractionCreateEventArgs e)
    {
        ...
        {
            WriteLine($"Message ID is not tracked or null. Message ID: {e.Message.Id}", CurrentStep.DiscordMessage,
                OutputType.Warning);
            return Task.CompletedTask;
        }
        ...
        {
            WriteLine("EmbedPages is zero, if you just started the bot, wait for the first refresh to be done.",
                CurrentStep.DiscordInteraction, OutputType.Warning);
            return Task.CompletedTask;
        }
        ...
        {
            WriteLine(
                $"Index out of range, page tracked index {pageTracked} is bigger than embed count {EmbedPages.Count}. Using the last index of the current embed pages",
                CurrentStep.DiscordInteraction, OutputType.Warning);
            pageTracked = EmbedPages.Count - 1;
        }

        var index = pageTracked;
        ...
        {
            WriteLine("No pages to show or page index out of range", CurrentStep.DiscordInteraction,
                OutputType.Warning);
            return Task.CompletedTask;
        }
        ...
        try
        {
            var uuids = GlobalServerInfo.Select(x => x.Uuid).ToList();
        ...
        {
            WriteLine("Interaction expired or already responded to. Skipping. " + nf.Message,
                CurrentStep.DiscordInteraction, OutputType.Error);
        }
        catch (BadRequestException br)
        {
            WriteLine("Bad request during interaction: " + br.JsonMessage, CurrentStep.DiscordInteraction,
                OutputType.Error);
        }
        catch (Exception ex)
        {
            WriteLine("Unexpected error during component interaction: " + ex.Message, CurrentStep.DiscordInteraction,
                OutputType.Error);
        }
    ...
    /// <summary>
    ///     Function that is called when the user interacts with the Start Button. It sends a start command to the Server in
    ///     question
    /// </summary>
    /// <param name="sender">DiscordClient</param>
    /// <param name="e">MessageDeleteEventArgs</param>
    /// <returns>Task of Type Task</returns>
    internal static async Task<Task> OnServerStartInteraction(DiscordClient sender,
    ...
    /// <summary>
    ///     Function that is called when the user interacts with the Stop Button. It sends a stop command to the Server in
    ///     question
    /// </summary>
    /// <param name="sender">DiscordClient</param>
    /// <param name="e">MessageDeleteEventArgs</param>
    /// <returns>Task of Type Task</returns>
    internal static async Task<Task> OnServerStopInteraction(DiscordClient sender,
        ComponentInteractionCreateEventArgs e)
    {
