...
public static class JsonHandler
{
    ...
    internal static int ExtractRconPort(string json, string uuid, string? variableName,
        List<ServerAllocation>? serverAllocations)
    {
        var rconPort = 0;
        if (variableName != null)
        {
            ...
            if (match.Success)
            {
                ...
                ConsoleExt.WriteLine($"[ExtractRconPort] Regex Extracted Rcon Port: {rconPort}",
        ...
        if (variableName is "SERVER_PORT")
        {
            ...
            ConsoleExt.WriteLine($"[ExtractRconPort] Server Port Extracted Rcon Port: {rconPort}",
                ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            return rconPort;
        }
        ...
        foreach (var data in serversArray)
        {
            ...
            ConsoleExt.WriteLine($"[ExtractRconPort] Server UUID: {serverUuid}", ConsoleExt.CurrentStep.JsonProcessing,
                ConsoleExt.OutputType.Debug);
            var variablesArray = data.GetPropertySafe("attributes").GetPropertySafe("relationships")
                .GetPropertySafe("variables").GetPropertySafe("data");
            ...
            foreach (var alloc in variablesArray.EnumerateArray())
            {
                ...
                if (attr.GetPropertySafe("env_variable").GetString() == variableName &&
                    ...
                    ConsoleExt.WriteLine($"[ExtractRconPort] Extracted Rcon Port: {rconPort}",
                        ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            }
        }
    ...
    internal static string ExtractRconPassword(string json, string uuid, string? variableName)
    {
        ...
        var rconPassword = string.Empty;
        var serversArray = root.GetPropertySafe("data").EnumerateArray();
        foreach (var data in serversArray)
        {
            var serverUuid = data.GetPropertySafe("attributes").GetPropertySafe("uuid").ToString();
            if (serverUuid != uuid) continue;
            ConsoleExt.WriteLine($"[ExtractRconPassword] Server UUID: {serverUuid}",
                ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            var variablesArray = data.GetPropertySafe("attributes").GetPropertySafe("relationships")
                .GetPropertySafe("variables").GetPropertySafe("data");
        ...
        List<ServerAllocation>? serverAllocations)
    {
        ...
        if (variableName != null)
        {
            ...
            if (match.Success)
            {
                ...
                ConsoleExt.WriteLine($"[ExtractQueryPort] Regex Extracted Query Port: {queryPort}",
                    ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
                return queryPort;
            }
        }
        ...
        if (variableName is "SERVER_PORT")
        {
            ...
            ConsoleExt.WriteLine($"[ExtractQueryPort] Server Port Extracted Query Port: {queryPort}",
                ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            return queryPort;
        }
        ...
        foreach (var data in serversArray)
        {
            var serverUuid = data.GetPropertySafe("attributes").GetPropertySafe("uuid").ToString();
            if (serverUuid != uuid) continue;
            ConsoleExt.WriteLine($"[ExtractQueryPort] Server UUID: {serverUuid}", ConsoleExt.CurrentStep.JsonProcessing,
                ConsoleExt.OutputType.Debug);
            var variablesArray = data.GetPropertySafe("attributes").GetPropertySafe("relationships")
                .GetPropertySafe("variables").GetPropertySafe("data");
            ...
            foreach (var alloc in variablesArray.EnumerateArray())
            {
                ...
                if (attr.GetPropertySafe("env_variable").GetString() == variableName &&
                    ...
                    ConsoleExt.WriteLine($"[ExtractQueryPort] Extracted Quert Port: {queryPort}",
                        ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            }
        }
    ...
    public static int ExtractMaxPlayerCount(string json, string uuid, string? variableName, string? maxPlayer)
    {
        if (!string.IsNullOrEmpty(maxPlayer))
            if (int.TryParse(maxPlayer, out var intMaxPlayers))
            {
                ConsoleExt.WriteLine($"[ExtractMaxPlayerCount] Max Player Count: {intMaxPlayers}",
                    ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
        ...
        foreach (var data in serversArray)
        {
            var serverUuid = data.GetPropertySafe("attributes").GetPropertySafe("uuid").ToString();
            if (serverUuid != uuid) continue;
            ConsoleExt.WriteLine($"[ExtractMaxPlayerCount] Server UUID: {serverUuid}",
                ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            var variablesArray = data.GetPropertySafe("attributes").GetPropertySafe("relationships")
                .GetPropertySafe("variables").GetPropertySafe("data");
            ...
            foreach (var alloc in variablesArray.EnumerateArray())
            {
                ...
                if (attr.GetPropertySafe("env_variable").GetString() == variableName &&
                    ...
                    ConsoleExt.WriteLine($"[ExtractMaxPlayerCount] Extracted Max Player Count: {maxPlayers}",
                        ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
            }
        }
    ...
    internal static ServerResources ExtractServerResources(string json)
    {
        ...
        var attributes = root.GetPropertySafe("attributes");
        ...
        var disk = resources.GetPropertySafe("disk_bytes").GetInt64();
        var networkRx = resources.GetPropertySafe("network_rx_bytes").GetInt64();
        var networkTx = resources.GetPropertySafe("network_tx_bytes").GetInt64();
        var uptime = resources.GetPropertySafe("uptime").GetInt64();
        ...
        ConsoleExt.WriteLine(
            $"[ExtractServerResources] Server Resources Extracted Current State: {currentState}, Memory Bytes: {memory}, CPU Abolute: {cpu}, Disk Bytes: {disk}, Network Rx Bytes {networkRx}, Network Tx Bytes: {networkTx}, Uptime: {uptime}",
            ConsoleExt.CurrentStep.JsonProcessing, ConsoleExt.OutputType.Debug);
    ...
    private static JsonElement? FindProperty(this JsonElement element, string propertyName)
    {
        if (element.ValueKind == JsonValueKind.Object)
            foreach (var property in element.EnumerateObject())
            {
                ...
                var found = property.Value.FindProperty(propertyName);
                if (found.HasValue)
                    return found;
            }
            ...
            {
                var found = item.FindProperty(propertyName);
                if (found.HasValue)
                    return found;
            }
    ...
    /// <summary>
    ///     Tries to get the Property in a JSON Element by name using the TryGetProperty function and falls back to
    ///     FindProperty if nothing was found on the first try
    /// </summary>
